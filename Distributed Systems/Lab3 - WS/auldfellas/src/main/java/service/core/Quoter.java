...
public class Quoter extends AbstractQuotationService  {
	...
	private static void jmdnsAdvertise(String host) {
		try {
			String config = "path=http://" + host + ":9001/quote?wsdl";
			JmDNS jmdns = JmDNS.create(InetAddress.getLocalHost());

			// Register a service
			ServiceInfo serviceInfo =
					ServiceInfo.create("_quote._tcp.local.", "auldfellas", 1234, config);
			...
			System.out.println("Problem Advertising auldfellas Service: " + e.getMessage());
			e.printStackTrace();
		}
	}
